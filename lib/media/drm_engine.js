...
shaka.media.DrmEngine = class {
  ...
  newInitData(initDataType, initData) {
    ...
    if (this.config_.enforceSingleSession && this.activeSessions_.size > 0) {
      shaka.log.debug('Enforcing single session, do not create new ones.');
      return;
    }

    shaka.log.debug('createSession:', initDataType, initData);
